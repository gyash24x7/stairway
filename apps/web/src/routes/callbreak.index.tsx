...
import { CreateGameDialog, JoinGameDialog } from "@callbreak/components";
...
export const Route = createFileRoute( "/callbreak/" )( {
	component: () => {
		...
		const navigateFn = ( gameId: string ) => navigate( { to: "/callbreak/$gameId", params: { gameId } } );
			...
			<div className={ "flex gap-5 flex-col mt-2" }>
				...
				{ !!authInfo ? (
					<div className={ "flex gap-5" }>
						<CreateGameDialog navigate={ navigateFn }/>
						<JoinGameDialog/>
					</div>
				) : (

...
export function Login() {
	...
	const passkeyLogin = async () => {
		...
		const optionsJSON = await getLoginOptions( { data: { username } } );
		const response = await startAuthentication( { optionsJSON } );
		await verifyLogin( { data: { username, response } } );
		await queryClient.invalidateQueries( { queryKey: [ "authInfo" ] } );
	};

	const passkeyRegister = async () => {
		const optionsJSON = await getRegisterOptions( { data: { username, name } } );
		const response = await startRegistration( { optionsJSON } );
		await verifyRegistration( { data: { username, name, response } } );
		await queryClient.invalidateQueries( { queryKey: [ "authInfo" ] } );
	};

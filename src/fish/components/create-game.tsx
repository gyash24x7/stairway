...
import { createGame } from "@/fish/server/functions";
import type { FishPlayerCount } from "@/libs/fish/types";
...
export function CreateGame() {
	...
	const [ playerCount, setPlayerCount ] = useState<FishPlayerCount>();
	const [ open, setOpen ] = useState( false );

	const handleClick = () => startTransition( async () => {
		const { error, data } = await createGame( { playerCount } );
		if ( !error && !!data ) {

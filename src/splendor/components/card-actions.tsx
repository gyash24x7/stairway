...
export function CardActions( props: CardActionsMenuProps ) {
	...
	const purchaseCardFn = useServerFn( purchaseCard );
	const purchaseMutation = useMutation( { mutationFn: purchaseCardFn } );
	...
	const reserveCardFn = useServerFn( reserveCard );
	const reserveMutation = useMutation( { mutationFn: reserveCardFn } );
			...
			<DialogContent>
				...
				<DialogFooter>
					{ ( type === "default" || type === "payment" ) && (
						...
						>
							{ purchaseMutation.isPending ? <Spinner/> : "PURCHASE" }
						</Button>
					) }
					{ ( type === "default" || type === "return" ) && (
						...
						>
							{ reserveMutation.isPending ? <Spinner/> : "RESERVE" }
						</Button>
					) }
				</DialogFooter>
			</DialogContent>

...
export const splendorEngine = new GameEngine( {
	name: "splendor",
	getNextPlayer: "round-robin",

	playerView: ( data, _config, playerId ): SplendorPlayerView => {
		const { decks, ...rest } = data;
	...
	moves: {
		pickTokens: {
			validate: ( state, _config, playerId, input: PickTokensInput ) => {
				...
				if ( typesPicked.length === 1 ) {
					...
					if ( typesPicked.some( gem => ( input.tokens[ gem ] ?? 0 ) > 1 ) ) {
						throw new Error( "You cannot pick more than 1 token of a type when picking 2 different types!" );
					}
					...
					if ( typesPicked.some( gem => ( input.tokens[ gem ] ?? 0 ) > 1 ) ) {
						throw new Error( "You cannot pick more than 1 token of a type when picking 3 different types!" );
					}
				} else {
				...
				if ( totalAfterPick <= MAX_TOKENS_IN_HAND ) {
					if ( input.returned ) {
						...
						if ( anyReturned ) {
							throw new Error( "You cannot return tokens when your total after pick does not exceed 10!" );
						}
					}
					return;
				}
			...
			execute: ( state, _config, playerId, input: PickTokensInput ) => {
		...
		reserveCard: {
			validate: ( state, _config, playerId, input: ReserveCardInput ) => {
				...
				if ( input.withGold ) {
					...
					if ( totalTokens + 1 > 10 && !input.returnedToken ) {
						throw new Error( "You must return a token when reserving with gold exceeds your token limit!" );
					}
					if ( totalTokens + 1 <= 10 && input.returnedToken ) {
						throw new Error(
			...
			execute: ( state, _config, playerId, input: ReserveCardInput ) => {
				logger.debug( ">> reserveCard()" );

				const player = state.data.playerData[ playerId ];
				const card = findCardInOpenCards( input.cardId, state.data.cards )!;

				player.reserved.push( card );
				const cardIdx = state.data.cards[ card.level ].findIndex( c => c?.id === card.id );
				state.data.cards[ card.level ][ cardIdx ] = state.data.decks[ card.level ].shift()!;
				...
				if ( input.returnedToken ) {
					...
					state.data.tokens[ input.returnedToken ] += 1;
				}
		...
		purchaseCard: {
			validate: ( state, _config, playerId, input: PurchaseCardInput ) => {
			...
			execute: ( state, _config, playerId, input: PurchaseCardInput ) => {
				logger.debug( ">> purchaseCard()" );

				const player = state.data.playerData[ playerId ];

				let fromReserved = false;
				let card = findCardInOpenCards( input.cardId, state.data.cards );
				if ( !card ) {
					card = player.reserved.find( card => card.id === input.cardId )!;
					fromReserved = true;
				}
				...
				if ( fromReserved ) {
					...
					const cardIdx = state.data.cards[ card.level ].findIndex( c => c && c.id === card.id );
					player.cards.push( card );
					state.data.cards[ card.level ][ cardIdx ] = state.data.decks[ card.level ].shift()!;
				}
				...
				Object.keys( input.payment ).map( g => g as Gem ).forEach( gem => {
					...
					state.data.tokens[ gem ] += input.payment[ gem ]!;
				} );

				for ( const noble of state.data.nobles ) {
					...
					if ( meetsRequirements ) {
						...
						state.data.nobles.splice( state.data.nobles.indexOf( noble ), 1 );
						break;
					}
				}
	...
	endIf: ( state, config ) => {
		...
		const anyoneWon = players.some( id =>
			state.data.playerData[ id ]?.points >= config.winningPoints
		);
		...
		const winner = players.reduce( ( best, id ) => {
			const points = state.data.playerData[ id ]?.points ?? 0;
			const bestPoints = state.data.playerData[ best ]?.points ?? 0;

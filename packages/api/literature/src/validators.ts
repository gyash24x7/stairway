...
import type { AskCardInput, CallSetInput, JoinGameInput, TransferTurnInput } from "./inputs";

const logger = createLogger( "LiteratureValidations" );

export async function validateJoinGame( input: JoinGameInput, authInfo: Auth.Info ) {
	...
	if ( game.players.length >= game.playerCount ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "The Game already has required players!" } );
	}
...
export async function validateCreateTeams( game: Literature.Game, players: Literature.PlayerData ) {
	...
	if ( Object.keys( players ).length !== game.playerCount ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "The Game doesn't have enough players!" } );
	}
...
export async function validateAskCard( input: AskCardInput, game: Literature.Game, players: Literature.PlayerData ) {
	...
	if ( !cardMapping ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "Card Not Part of Game!" } );
	}
	...
	if ( !askedPlayer ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "The Player is not part of the Game!" } );
	}

	if ( playerWithAskedCard.id === game.currentTurn ) {
		logger.debug( "The asked card is with asking player itself! GameId: %s", game.id );
		throw new TRPCError( { code: "BAD_REQUEST", message: "The asked card is with asking player itself!" } );
	}
	...
	if ( players[ game.currentTurn ].teamId === askedPlayer.teamId ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "The asked player is from the same team!" } );
	}
...
export async function validateCallSet( input: CallSetInput, game: Literature.Game, players: Literature.PlayerData ) {
	...
	if ( !Object.values( input.data ).includes( game.currentTurn ) ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "Calling Player did not call own cards!" } );
	}

	if ( cardSets.size !== 1 ) {
		logger.error( "Cards Called from multiple sets! UserId: %s", game.currentTurn );
		throw new TRPCError( { code: "BAD_REQUEST", message: "Cards Called from multiple sets!" } );
	}
	...
	if ( !isCardSetWithCallingPlayer ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "Set called without cards from that set!" } );
	}
	...
	if ( calledTeams.size !== 1 ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "Set called from multiple teams!" } );
	}

	if ( calledCards.length !== 6 ) {
		logger.error( "All Cards not called for the set! UserId: %s, Set: %s", game.currentTurn, calledSet );
		throw new TRPCError( { code: "BAD_REQUEST", message: "All Cards not called for the set!" } );
	}
...
export async function validateTransferTurn(
	...
	if ( !lastMove ) {
		...
		throw new TRPCError( {
			code: "BAD_REQUEST",
			message: "Turn can only be transferred after a successful call!"
		} );
	}
	...
	if ( !receivingPlayer ) {
		...
		throw new TRPCError( { code: "BAD_REQUEST", message: "The Receiving Player is not part of the Game!" } );
	}

	if ( cardCounts[ input.transferTo ] === 0 ) {
		logger.error( "Turn can only be transferred to a player with cards!" );
		throw new TRPCError( { code: "BAD_REQUEST", message: "Turn can only be transferred to a player with cards!" } );
	}

	if ( receivingPlayer.teamId !== transferringPlayer.teamId ) {
		logger.error( "Turn can only be transferred to member of your team!" );
		throw new TRPCError( { code: "BAD_REQUEST", message: "Turn can only be transferred to member of your team!" } );
	}

...
import type { AuthService, JwtService } from "../../src/services";
...
describe( "AuthMiddleware", () => {
	...
	mockRes.locals[ Constants.AUTH_INFO_ID ] = undefined;
	...
	it( "should call next if no token without setting user id", async () => {
		...
		expect( mockRes.locals[ Constants.AUTH_INFO_ID ] ).toBeUndefined();
	} );
	...
	it( "should read access token and call next with the user id set in locals", async () => {
		...
		expect( mockNextFn ).toHaveBeenCalledTimes( 1 );
		expect( mockRes.locals[ Constants.AUTH_INFO_ID ] ).toBe( subject );
	} );

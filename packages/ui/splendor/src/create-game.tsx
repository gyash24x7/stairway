...
import { useCreateGameMutation } from "@s2h/client/splendor";
...
export function CreateGame() {
	...
	const { mutateAsync, isPending } = useCreateGameMutation( {
		onSuccess: ( { gameId } ) => navigate( { to: `/splendor/${ gameId }` } )
	} );

	const handleClick = () => mutateAsync( { playerCount: playerCount ?? 2 } );
